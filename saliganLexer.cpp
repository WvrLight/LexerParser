...
//					writeToken(token, "delimiter");
		...
		// NUMBERS
			...
			if (isFloat == false) checkToken(&state, token, tokenSize, "integer");
			else checkToken(&state, token, tokenSize, "float");
...
void Lexer::clearToken(char token[], int *tokenSize, int *tokenType) {
	memset(&(token[0]), 0, MAXTOKENSIZE);
	*tokenSize = 0;
	*tokenType = 0;
}

char Lexer::getNextChar(char *ch) {
	if (!file.eof()) {
		*ch = file.get();
		return *ch;
	}
...
void Lexer::checkToken(bool *state, char token[], int tokenSize, string type) {
	*state = true;
...
void Lexer::writeToken(char token[], string type) {
	symbolTable << setw(10) << left << token << TAB << type << endl;
